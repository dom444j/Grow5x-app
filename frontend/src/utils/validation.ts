...
// BEP20/Ethereum address validation
...
export const validateTxHash = (txHash: string): boolean => {
  ...
  // Transaction hash: 64 characters hexadecimal (with or without 0x prefix)
  const txHashRegex = /^(0x)?[a-fA-F0-9]{64}$/;
  return txHashRegex.test(cleanHash);
};

// Amount validation for USDT transactions
export const validateUSDTAmount = (amount: string | number, min: number = 0.01, max: number = 1000000): { isValid: boolean; error?: string } => {
  ...
  if (numAmount < min) {
    return { isValid: false, error: `El monto mínimo es ${min} USDT` };
  }
  
  if (numAmount > max) {
    return { isValid: false, error: `El monto máximo es ${max.toLocaleString()} USDT` };
  }
  
  // Check for reasonable decimal places (max 6 for USDT)
  const decimalPlaces = (numAmount.toString().split('.')[1] || '').length;
  if (decimalPlaces > 6) {
    return { isValid: false, error: 'Máximo 6 decimales permitidos' };
  }
...
// Network validation
export const validateNetwork = (network: string): { isValid: boolean; error?: string } => {
  if (!network || typeof network !== 'string') {
    return { isValid: false, error: 'Red requerida' };
  }
  
  const validNetworks = ['BEP20', 'ERC20', 'TRC20'];
  const cleanNetwork = network.trim().toUpperCase();
  
  if (!validNetworks.includes(cleanNetwork)) {
    return { isValid: false, error: `Red no soportada. Redes válidas: ${validNetworks.join(', ')}` };
  }

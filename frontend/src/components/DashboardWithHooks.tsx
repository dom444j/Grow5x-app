...
import { useMe, useMyPurchases, useMyWithdrawals } from '../hooks';
...
const ReferralCodeCard: React.FC<ReferralCodeCardProps> = ({ referralCode, totalReferrals, loading }) => {
...
const QuickStats: React.FC<QuickStatsProps> = ({ purchases, withdrawals, loading }) => {
  if (loading) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-secondary-200 p-6">
        <div className="animate-pulse">
        ...
        <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <div className="flex items-center">
            ...
            <div className="text-sm">
              ...
              <span className="text-yellow-700"> en retiros pendientes</span>
            </div>
          </div>
        </div>
      )}
...
const DashboardWithHooks: React.FC = () => {
  ...
  const { user: userData, loading: userLoading, error: userError } = useMe();
  const { purchases, loading: purchasesLoading } = useMyPurchases();
  const { withdrawals, loading: withdrawalsLoading } = useMyWithdrawals();
  ...
  const isLoading = userLoading || purchasesLoading || withdrawalsLoading;
    ...
    <UserLayout title="Dashboard">
      <div className="bg-gradient-to-br from-primary-50 to-secondary-50 -mx-4 -my-6 px-4 py-6 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 min-h-screen">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
            <div className="flex items-center">
              ...
              <p className="text-red-700 font-medium">{(userError as any)?.message || (typeof userError === 'string' ? userError : 'Error desconocido')}</p>
            </div>
          </div>
        )}
            ...
            {userData && (
              <div className="flex flex-col items-end space-y-2">
                ...
                {userData.role && userData.role !== 'user' && (
                  <div className="flex items-center space-x-2">
                    <span className="text-sm text-secondary-600">Rol:</span>
                    <span className="px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 capitalize">
                      {userData.role === 'admin' ? 'Administrador' : userData.role === 'support' ? 'Soporte' : userData.role}
                    </span>
                  </div>
                )}
                ...
                {userData.createdAt && (
                  <div className="flex items-center space-x-2">
                    <span className="text-sm text-secondary-600">Miembro desde:</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <KPICard
            title="Balance Total"
            value={userData?.balances?.total ? `$${userData.balances.total.toLocaleString()}` : '$0.00'}
            icon={
          ...
          <KPICard
            ...
            value={purchases?.filter(p => p.status === 'confirmed').length || 0}
            icon={
          ...
          <KPICard
            ...
            value={withdrawals?.filter(w => w.status === 'pending').length || 0}
            icon={
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          <ReferralCodeCard
            referralCode={userData?.referralCode || 'LOADING...'}
          ...
          <QuickStats
            ...
            loading={purchasesLoading || withdrawalsLoading}
          />
        </div>

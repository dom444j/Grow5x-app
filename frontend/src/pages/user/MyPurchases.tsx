...
import { useMyPurchases, useMyBenefits } from '../../hooks';
...
const MyPurchases: React.FC = () => {
  ...
  const { benefits, loading: benefitsLoading, error: benefitsError, refetch: refetchBenefits } = useMyBenefits();
  const [copiedHash, setCopiedHash] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'purchases' | 'licenses'>('purchases');
  ...
  const handleRefresh = async () => {
    await Promise.all([refetch(), refetchBenefits()]);
  };
    ...
    <UserLayout title="Mis Compras">
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-secondary-900">Mis Compras y Licencias</h2>
          <button
            onClick={handleRefresh}
            disabled={loading || benefitsLoading}
        ...
        {/* Tabs - Solo mostrar si hay beneficios activos */}
        {benefits?.summary.activeBenefitSchedules > 0 ? (
          <div className="flex space-x-1 mb-6 bg-gray-100 p-1 rounded-lg">
            ...
            >
              <div className="flex items-center justify-center gap-2">
                ...
                Licencias en Producción
                <span className="bg-green-100 text-green-600 text-xs px-2 py-1 rounded-full">
                  {benefits.summary.activeBenefitSchedules}
                </span>
              </div>
            </button>
          </div>
          ...
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-secondary-900 mb-2">Historial de Compras</h3>
            <p className="text-sm text-secondary-600">Aquí puedes ver todas tus compras. Las licencias aparecerán cuando tus compras sean confirmadas y activadas.</p>
          </div>
        )}
        
        {/* Tab Content */}
        {(activeTab === 'purchases' || !benefits?.summary.activeBenefitSchedules) && (
          <div>
          ...
          <div className="space-y-6">
            {purchases.map((purchase: any, index: number) => {
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-secondary-900 mb-2">No tienes compras aún</h3>
                <p className="text-secondary-600">Cuando realices tu primera compra, aparecerá aquí.</p>
              </div>
            )}
          </div>
        )}
        ...
        {/* Licenses Tab */}
        {activeTab === 'licenses' && benefits?.summary.activeBenefitSchedules > 0 && (
          <div>
            {benefitsLoading ? (
              <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="animate-pulse">
              ...
              <div className="text-center py-12">
                <FiX className="w-16 h-16 text-red-400 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-secondary-900 mb-2">Error al cargar licencias</h3>
                <p className="text-secondary-600 mb-4">{benefitsError.message}</p>
                <button
            ...
            ) : benefits && benefits.benefitSchedules.length > 0 ? (
              <div>
                {/* Summary Cards */}
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                  <div className="bg-gradient-to-r from-green-50 to-green-100 border border-green-200 rounded-lg p-4">
                    <div className="flex items-center text-green-600 mb-2">
                      ...
                      <span className="text-sm font-medium">Total Liquidado</span>
                    </div>
                    <p className="text-2xl font-bold text-green-700">
                      ${toRenderableNumber(benefits.summary.totalBenefits)}
                    </p>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-purple-50 to-purple-100 border border-purple-200 rounded-lg p-4">
                    ...
                    <p className="text-2xl font-bold text-purple-700">
                      {benefits.summary.completedBenefitSchedules}
                    </p>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-orange-50 to-orange-100 border border-orange-200 rounded-lg p-4">
                    ...
                    <p className="text-2xl font-bold text-orange-700">
                      ${toRenderableNumber(benefits.summary.totalCommissions)}
                    </p>
                  </div>
                </div>
                ...
                {/* License Trackers */}
                <div className="space-y-6">
                  <h3 className="text-xl font-semibold text-gray-900 mb-4">Seguimiento de Licencias</h3>
                  {benefits.benefitSchedules.map((schedule) => (
                    <LicenseTracker key={schedule.scheduleId} schedule={schedule} />
                  ))}
                </div>
              </div>
            ) : (
              <div className="text-center py-12">
                <FiTrendingUp className="w-16 h-16 text-secondary-400 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-secondary-900 mb-2">No tienes licencias en producción</h3>
                <p className="text-secondary-600">Cuando tus compras sean confirmadas y activadas, aparecerán aquí como licencias en producción.</p>
              </div>
            )}
          </div>
        )}

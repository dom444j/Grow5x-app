...
router.get('/', reportsLimiter, authenticateToken, requireAdmin, validateDateRange, async (req, res) => {
  try {
    ...
    // BENEFICIOS personales (ledger)
    const benefits = await Ledger.aggregate([
      { $match: { createdAt: { $gte: start, $lte: end }, type: 'BENEFIT_ACCRUAL' } },
      { $group: { _id: dayStr(tz), total: { $sum: '$amount' }, count: { $sum: 1 } } },
      { $sort: { _id: 1 } },
    ]);
...
router.get('/sales', reportsLimiter, authenticateToken, requireAdmin, validateDateRange, async (req, res) => {
  try {
...
router.get('/referrals', reportsLimiter, authenticateToken, requireAdmin, validateDateRange, async (req, res) => {
  try {
    const { from, to } = req.dateRange;
...
router.get('/benefits', reportsLimiter, authenticateToken, requireAdmin, validateDateRange, async (req, res) => {
  try {
    const { from, to } = req.dateRange;
...
router.get('/withdrawals', reportsLimiter, authenticateToken, requireAdmin, validateDateRange, async (req, res) => {
  try {
    const { from, to } = req.dateRange;

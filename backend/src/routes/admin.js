...
router.use('/import-jobs', userImportRoutes);
  ...
  try {
    ...
    const transformedPurchases = purchases.map(purchase => ({
      ...
      user: {
        ...
        fullName: `${purchase.userId.firstName} ${purchase.userId.lastName}`
      },
      package: {
        packageId: purchase.packageId.packageId,
        name: purchase.packageId.name,
        price: toApiNumber(purchase.packageId.price)
      },
      payment: {
        quantity: purchase.quantity,
        unitPrice: toApiNumber(purchase.unitPrice),
        totalAmount: toApiNumber(purchase.totalAmount),
      ...
      transaction: {
        ...
        submittedAt: purchase.paymentSubmittedAt
      },
      status: purchase.status,
      createdAt: purchase.createdAt,
      waitingTime: DecimalCalc.round((new Date() - purchase.hashSubmittedAt) / (1000 * 60)) // minutes
    }));
...
router.get('/purchases/all', async (req, res) => {
  try {
    ...
    const transformedPurchases = purchases.map(purchase => ({
      ...
      timestamps: {
        createdAt: purchase.createdAt,
        paymentSubmittedAt: purchase.paymentSubmittedAt,
      ...
      adminActions: {
        confirmedBy: purchase.confirmedBy ? {
          userId: purchase.confirmedBy.userId,
          fullName: `${purchase.confirmedBy.firstName} ${purchase.confirmedBy.lastName}`
        } : null,
        adminNotes: purchase.adminNotes
      },
      waitingTime: purchase.paymentSubmittedAt ? 
        DecimalCalc.round((new Date() - purchase.paymentSubmittedAt) / (1000 * 60)) : null // minutes
    }));
...
router.get('/overview', async (req, res) => {
  try {
    ...
    // Sales metrics
    const [sales24h, sales7d] = await Promise.all([
      Purchase.aggregate([
        {
          $match: {
            status: 'confirmed',
    ...
    // Referrals metrics
    const [directReferrals24h, parentGlobalQueued, parentGlobalReleased24h] = await Promise.all([
      Ledger.aggregate([
        {
          $match: {
            type: 'REFERRAL_DIRECT',
            status: 'confirmed',
            transactionDate: { $gte: last24h }
          }
        },
        {
          $group: {
    ...
    // Withdrawals metrics
    const [withdrawalsPending, withdrawalsApproved, withdrawalsCompleted24h] = await Promise.all([
    ...
    // SLA Hit Rate calculation (7 days)
    const withdrawalsSLA = await Withdrawal.aggregate([
      {
        $match: {
          status: 'completed',
    ...
    const slaHitRate7d = slaData ? (slaData.withinSLA / slaData.total) : 0;

    const response = {
      asOf: now.toISOString(),
      sales: {
        count24h: sales24h[0]?.count || 0,
        amount24hUSDT: DecimalCalc.round((sales24h[0]?.amount || 0), 2),
        count7d: sales7d[0]?.count || 0,
        amount7dUSDT: DecimalCalc.round((sales7d[0]?.amount || 0), 2)
      },
      referrals: {
        direct24hUSDT: DecimalCalc.round((directReferrals24h[0]?.amount || 0), 2),
    parentGlobalQueuedUSDT: DecimalCalc.round((parentGlobalQueued[0]?.amount || 0), 2),
    parentGlobalReleased24hUSDT: DecimalCalc.round((parentGlobalReleased24h[0]?.amount || 0), 2)
      },
      benefits: {
      ...
      withdrawals: {
        ...
        slaHitRate7d: DecimalCalc.round(slaHitRate7d * 100, 1) // Percentage with 1 decimal
      },
      poolV2: {
...
/**
  ...
  try {
    ...
    const benefitsToday = await Ledger.aggregate([
      {
        $match: {
          type: 'BENEFIT_ACCRUAL',

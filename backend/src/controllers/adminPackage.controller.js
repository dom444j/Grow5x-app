...
exports.getAllPackages = async (req, res) => {
  try {
    ...
    const formattedPackages = packages.map(pkg => {
      // Fix features format - convert malformed objects to proper strings
      let formattedFeatures = [];
      if (pkg.features && Array.isArray(pkg.features)) {
        formattedFeatures = pkg.features.map(feature => {
          if (typeof feature === 'string') {
            return feature;
          } else if (feature && typeof feature === 'object') {
            // If it's an object with numeric keys, reconstruct the string
            if (feature.name) {
              return feature.name;
            } else {
              // Reconstruct from numeric keys
              const keys = Object.keys(feature).filter(key => !isNaN(key)).sort((a, b) => parseInt(a) - parseInt(b));
              return keys.map(key => feature[key]).join('');
            }
          }
      ...
      return {
        ...
        images: pkg.images,
        createdBy: pkg.metadata?.createdBy,
        lastModifiedBy: null, // Campo no existe en el schema
...
exports.getPackageById = async (req, res) => {
  try {
    ...
    if (package.features && Array.isArray(package.features)) {
      formattedFeatures = package.features.map(feature => {
        if (typeof feature === 'string') {
          return feature;
        } else if (feature && typeof feature === 'object') {
          // If it's an object with numeric keys, reconstruct the string

...
/**
 * Valida que una dirección sea válida para BEP20 (formato Ethereum)
 * @param {string} address - Dirección a validar
 * @returns {boolean} - true si es válida
 */
function validateUSDTBEP20Address(address) {
  if (!address || typeof address !== 'string') {
    return false;
  }
  
  // Formato BEP20 (mismo que Ethereum): 0x seguido de 40 caracteres hexadecimales
  const bep20Regex = /^0x[a-fA-F0-9]{40}$/;
  return bep20Regex.test(address);
}

/**
 * Valida que la red sea BEP20
 * @param {string} network - Red a validar
 * @returns {boolean} - true si es BEP20
 */
function validateUSDTBEP20Network(network) {
  return network === ALLOWED_CONFIG.network;
}

/**
 * Valida que la moneda sea USDT
 * @param {string} currency - Moneda a validar
 * @returns {boolean} - true si es USDT
 */
function validateUSDTBEP20Currency(currency) {
  return currency === ALLOWED_CONFIG.currency;
}

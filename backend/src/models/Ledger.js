...
const ledgerSchema = new mongoose.Schema({
  ...
  type: {
    ...
    enum: [
      ...
      'BENEFIT_ACCRUAL',    // Daily benefit release (12.5%)
...
ledgerSchema.statics.createBenefitAccrual = async function({
  userId,
  amount,
  purchaseId,
  benefitScheduleId,
  ...
  const benefitIdempotencyKey = this.generateBenefitIdempotencyKey(purchaseId, day);
  ...
  const entry = new this({
    ...
    currency: 'USDT',
    balance: newBalance,
    description: description || `Daily benefit accrual - Day ${day + 1}`,
    references: {
      purchaseId,
      benefitScheduleId
    },
